...
> 当前版本：v0.6.1。Rust 主程序已按职责拆分：`main.rs`（入口/共享类型）、`cli.rs`（参数解析与分发）、`config.rs`（配置与凭据）、`editor.rs` / `editor/`（可视化编辑器路由与生命周期）、`daemon.rs`（daemon 与连接池）、`transfer.rs`（SFTP 传输）、`ssh.rs`（建连认证）、`exec.rs`（命令执行与提权编排）、`privilege.rs`（提权命令字符串）、`runtime.rs`（runtime/超时封装）。

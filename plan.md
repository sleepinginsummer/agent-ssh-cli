...
> 当前版本：v0.5.7。Rust 主程序已按职责拆分：`main.rs`（CLI/配置/共享类型）、`daemon.rs`（daemon 与连接池）、`transfer.rs`（SFTP 传输）、`ssh.rs`（建连认证）、`exec.rs`（命令执行与提权编排）、`privilege.rs`（提权命令字符串）、`runtime.rs`（runtime/超时封装）。

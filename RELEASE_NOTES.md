...
- 将仅用于承载配置输入的 `<form>` 改为普通容器，用户名不再声明为网页登录账号，减少浏览器密码管理器的误判；第三方扩展仍可能自行弹窗。
- 移除多余的“替换密码”按钮，直接在密码框输入即可创建新密码草稿；查看已保存密码后聚焦输入框会先清除临时明文，只有实际输入才产生草稿。

验证：

- `npm test` 通过：平台分支、编辑器前端契约与 75 项 Rust 测试。
- `npm run build:native` release 构建通过，原生程序 `--version` 输出 `0.6.2`。
- 本地预览页验证直接输入生成草稿、查看状态聚焦清除明文且不新增草稿；未连接真实服务器。

## v0.6.1

改进可视化连接编辑器并修复 SSH 测试与服务器身份校验：

- 连接列表在新增、删除、搜索和 JSON 调整顺序时保留滚动位置与焦点；未保存连接显示标记，支持撤销本次改动。
- 新增“测试连接”：使用页面草稿（含临时密码和跳板机配置）进行一次 SSH 建连与认证，不保存配置、不执行远端命令；单次最长 15 秒，并发测试受限，其他编辑器请求保持响应。
- 修复连接名含首尾空格时测试连接查找失败的问题。
- 所有新建 SSH 会话（含测试、命令、传输及跳板机）在认证前核对 `~/.ssh/known_hosts` 中的服务器公钥；未知或变化的公钥直接拒绝，不自动登记。首次连接需先独立核实并登记服务器公钥。

验证：

- `npm test` 通过：平台分支与编辑器契约检查、75 项 Rust 测试（含本地 SSH 成功认证及公钥匹配/变更/未知主机测试）。
- `npm run build:native` release 构建通过，原生程序 `--version` 输出 `0.6.1`。
- 未连接真实远端服务器执行认证测试。
...
- 修复前：远端目录不存在时上传报 `创建远端续传元数据失败: No such file: No such file`，且会白跑 3 次重试，看不出真正原因。
- 修复后：上传前校验父目录，直接返回 `远端目录不存在或不可访问: <dir>（单文件上传不会自动创建目录，请先创建该目录，或改用 --recursive 上传整个目录）`，且不再重试。
- 父路径存在但是同名文件的情况同样被拦下（用 `metadata().is_dir()` 判定，而非只看 metadata 是否成功）。
- 校验复用重试循环首次打开的那个 SFTP 会话，不额外开 subsystem；递归上传逐文件时开销可控（每文件仅多一次 stat）。
- 远端路径不含目录部分（如 `a.txt`，等价于会话当前工作目录）时跳过校验；`--recursive` 目录上传行为不变（仍会自动创建远端目录）。
- 文档同步：README「常用参数」与 SKILL 的 upload 段补充该行为说明。

验证：

- `npm test` 通过：平台分支检查 + 42 项测试（新增 `remote_parent_path` 用例，覆盖绝对/相对/根/无目录/尾斜杠）。
- `cargo clippy` 0 error，警告 6 项（均为既有风格项）。
- 真机（web正式1）：父目录不存在、父路径是文件、目录存在、递归多文件上传、相对路径、daemon 缓存模式 6 个场景结果均符合预期，远端文件清单核对一致。
## v0.5.6
